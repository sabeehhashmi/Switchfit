  ...
  <div class="row">
    <div class="col">
      <div class="page-title-box">
        <h4 class="page-title">{{\App\User::getFullName(getUrlSegment(5))}}'s Payouts </h4>
      </div>
    </div>
      ...
      <div class="card">
        <div class="card-body">
          ...
          <form class="dates-conatiner-form">
            {{ csrf_field() }}
            <input type="hidden" name="gym_owner_id" class="tariner_id" value="{{getUrlSegment(5)}}">
            <div class="row">
...
<script>
  ...
  $(document).ready(function () {
    $('.btn-danger').click(function(){
      ...
      $('.to_date').val('');
      $.ajax({
        url: '{{url('/')}}'+'/gym_owner/list/gyms/payout/search',
        type: "POST",
        data: {
          _token: '{{csrf_token()}}',
          gym_owner_id:$('.tariner_id').val(),
    ...
    $('.btn-download-xlx').click(function(){
      formData = $('.dates-conatiner-form').serialize();
      $.ajax({
        url: '{{url('/')}}'+'/gym_owner/list/gyms/payout/download',
        type: "POST",
        data: {
          _token: '{{csrf_token()}}',
          gym_owner_id:$('.tariner_id').val(),
